    ...
    <section className="container-app py-20">
      <div className="grid gap-10 lg:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-6 flex items-center gap-3"><span className="text-3xl font-black text-slate-950">{price}</span>{product.promo_price ? <span className="text-base text-slate-400 line-through">{formatCurrency(product.price)}</span> : null}</div>
          <div className="mt-8 space-y-3">{product.features?.map((feature) => <div key={feature} className="flex items-center gap-3 text-slate-700"><CheckCircle2 className="text-brand" size={18} /><span>{feature}</span></div>)}</div>
