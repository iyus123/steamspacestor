  ...
  return (
    <form action={action} className="grid gap-5 md:grid-cols-2">
      ...
      <div className="md:col-span-2">
        ...
        <textarea name="advantages" defaultValue={homepage.advantages.join("
")} className="textarea min-h-[140px]" />
      </div>

      {[1, 2, 3].map((index) => (
        <div key={`product-${index}`}>
          <label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Produk populer {index}</label>
          <select name={`featured_product_id_${index}`} defaultValue={featuredProducts[index - 1]} className="input">
      ...
      {[1, 2, 3].map((index) => (
        <div key={`testimonial-${index}`}>
          <label className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">Testimoni homepage {index}</label>
          <select name={`featured_testimonial_id_${index}`} defaultValue={featuredTestimonials[index - 1]} className="input">
            <option value="">Pilih testimoni</option>
            {testimonials.map((item) => (
              <option key={item.id} value={item.id}>{item.customer_name} — {item.role}</option>
            ))}
          </select>
        </div>
      ))}
